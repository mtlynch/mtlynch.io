...
I'm going to make the bold assumption that you want to improve code reviews in the present, where your teammates are humans. I'll make the even wilder assumption that a positive relationship with your colleagues is an end in itself and not simply a variable you adjust to minimize your cost per defect discovery. How would your code review practices change under those circumstances?
...
In this article, I discuss code review techniques that treat a code review as not only a technical process, but a social one as well.
...
Arguments about style are a waste of time in code reviews. Consistent style is certainly important, but a code review is not the time to argue about where to put the curly braces. The best way to excise style debates from your code reviews is by keeping a style guide.
...
Treat code reviews as high priority. When you're actually reading the code and giving feedback, take your time, but *start* your review immediately — ideally, within minutes.

{% include image.html file="relay.png" alt="A code review relay race" max_width="600px" img_link=true %}

If a teammate sends you a changelist, it likely means that they are blocked on other work until your review is done. In theory, source control systems allow the author to branch, continue working, then forward-merge changes from the review into their new branch. In reality, there are like four people total who know how to do that without wasting time fighting conflicts from three-way diffs.
...
The more notes  you write in a given review round, the more you risk making the author feel overwhelmed.  The exact limit will vary by developer, but the danger zone generally begins in the range of 20-50 notes in a single round of review.
...
If you lighten the author's load by actually writing some of the changes you're suggesting, you demonstrate that you are generous with your time as a reviewer.
...
Code reviews require more tact and care than usual communication because there's a high risk of derailing the discussion into a personal argument. You would expect reviewers to dial up their politeness in reviews, but bizarrely I've found the opposite to be true.  Most people never say to a co-worker, "Hand me that stapler, then fetch me a soda," but I've seen numerous code reviewers frame feedback with similarly pushy commands, such as, "Move this class to a separate file."
...
Requests also make it easier for the author to push back politely. Maybe they have a good reason for their choice. If you frame your feedback as a command, any pushback from the author comes across as disobeying your order. If you frame your feedback as a request or a question, the author can simply answer you.
...
When you give the author a note, explain both your suggested change and the *reason* for the change. Instead of saying, "We should split this class into two," it's much better to say, "Right now, this class is responsible for both downloading the file and parsing it. We should split it up into a downloader class and a parsing class per the [single responsibility principle](https://en.wikipedia.org/wiki/Single_responsibility_principle)."
