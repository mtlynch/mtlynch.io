...
I've been reading articles about code review latetly and I've noticed one thing they all have in common.  They focus exclusively on finding bugs and say nothing about communication. They're describing I've noticed that articles about best practices for code review tend to focus exclusively on finding bugs and don't say much about actually communicating those issues. I get it, because when you're describing a process in which one person is critiquing the work, what could possibly be worth discussing about the communication?

I realized all the established best practices for code reviews apply equally well to managing romantic relationships, so I wrote a new ebook to help developers with their love life:
...
I'm going to make a big assumption and say that you want to improve code reviews in the present, where your teammates are humans. I'll make the even wilder assumption that a positive relationship with your colleagues is an end in itself and not simply a variable you adjust to minimize your cost of defect discovery. How would you design effective code review practices under these assumptions?

I was only able to find one author who stressed the importance of social and cultural factors in code reviews. In his book, [*Peer Reviews in Software: A Practical Guide*](http://amzn.to/2xw6AWV), author Karl E. Wiegers illustrates this factor eloquently:
...
In this article, I'll discuss some code review techniques that treat a code review as not only a technical process, but a social one as well.
...
If a programmer sends you a changelist that they think is awesome, and you write them back with an extensive list of reasons why it's not, that's a sensitive message to get across.
...
It's very easy for an author to interpret criticism of their code as suggestion that they are an incompetent programmer. Code reviews are an opportunity to share knowledge and make informed engineering decisions, but that can't happen if the author perceives the discussion as a personal attack.

As if this wasn't hard enough, you also have the challenge of communicating your criticism in writing, where the risk of miscommunication is much higher. The author can't hear your voice or see your body language, so it's even more important to be careful and purposeful in how you articulate your feedback. To an author who's feeling defensive, an innocuous note like, "You forgot to close the file handle," can be read as, "I can't *believe* you forgot to close the file handle! You're such an idiot."
...
As a developer, the time you can spend focused on code is scarce. Your mental stamina is scarcer still. Reading a teammate's code is cognitively taxing and requires a high level of concentration. Don't squander these resources on tasks a computer can do, especially when a computer can do them better.
...
Arguments about style are a waste of time in code reviews. There's tremendous value in keeping style consistent across your codebase, but a code review is not the time you want to be arguing which line the curly braces start on. The best way to excise style debates from your code reviews is by keeping a style guide.
...
The more notes  you write in a given review round, the higher your risk of making the author feel overwhelmed.  The exact limit will vary by developer, but the danger zone generally begins in the range of 20-50 notes in a single round of review.
...
In an ideal world, the code author would be thankful for every review they receive. It's an opportunity for them to learn, and it protects them from their mistakes. In reality, there are a number of external factors that could cause the author to view the experience negatively and to resent you for giving them notes. Maybe they're under deadline pressure, so anything other than your instant, rubber-stamp approval feels like you're being obstructive. Maybe you haven't worked together much, so they don't trust that your feedback is meant to be helpful.

A great way to make an author feel good about the code review process is to find opportunities to give them gifts during the review. And what's the gift all developers love to receive? Code examples, of course.

{% include image.html file="code-gift.png" alt="Receiving the gift of code" max_width="600px" img_link=true %}

If you lighten the author's load by actually writing some of the changes you're suggesting, you demonstrate to the author that you are generous with your time as a reviewer.
...
The decisions you reach in a code review should be based on what makes the code better rather than who came up with the idea. Your teammate put significant effort into their changelist and is likely proud of the work they did. Their natural reaction to hearing criticism of their work is to feel defensive and protective.

Word your feedback in a way that minimizes the risk of raising your teammate's defenses. Be clear that you're critiquing the code, not the coder. When an author sees "you" in a comment, it brings their focus away from the code and back to themselves, and increases the risk that they'll take your criticism personally.

Consider a harmless comment like the following:
...
Contrast this with the same correction without "you":

>sucessfully -> successfully

The latter note reads much more obviously as a simple correction and not a judgment of the author.
...
"We" reinforces the team's collective responsibility for the code. The author may move on and so might you, but the team will remain in one form or another. In some cases, it sounds silly to say "we" when it's clearly something you expect the author to do themselves, but I'd rather my notes sound silly than accusatory.
...
The  [passive voice](https://en.wikipedia.org/wiki/English_passive_voice) achieves a similar effect. I generally avoid the passive voice like the plague in my technical writing, but it can be a helpful way of writing around "you":

>This variable **should be renamed** to something more descriptive, like `seconds_remaining`.

## Frame feedback as requests, not commands

Code reviews require more tact and care than usual communication because there's a high risk of derailing the engineering discussion into a personal argument. You would reviewers to dial up their politeness in reviews, but bizarrely I've found the opposite to be true.  Most people would never say to a co-worker, "Hand me that stapler, then fetch me a soda," but I've seen numerous code reviewers frame feedback with similarly pushy commands, such as, "Move this class to a separate file."
...
Requests also make it easier for the author to push back politely. Maybe they have a good reason for their choice. If you frame your feedback as a command, pushing back requires them to disobey your order. If you frame your feedback as a request or a question, they can simply answer your question.
...
When you give the author a note, you should not only suggest a change, but the *reason* for the change. For example, instead of saying, "We should split this class into two," it's much better to say, "Right now, this class is responsible for both downloading the file and parsing it. We should split it up into a downloader class and a parsing class per the [single responsibility principle](https://en.wikipedia.org/wiki/Single_responsibility_principle)."

Grounding your notes in principles frames the discussion in a constructive way. When you cite a specific reason, like "We should make this function private to minimize the class's public interface," the author can't simply respond, "No, I prefer it my way." Or rather, they *can*, but it would look silly because you demonstrated how the change satisfies a goal, and they just stated a preference.

Software development is both an art and a science. In some cases, you can't articulate what exactly is wrong with a piece of code in terms of established principles. Some code is just ugly or unintuitive and it's hard to pin down why. In these cases, explain what you can, but keep it objective.  If you say, "**I** found this hard to understand," that's at least an objective statement, as opposed to "**this is** confusing," which is a value judgment and may not be true for every person.

Provide supporting evidence where possible in the form of links.  The relevant section of the team style guide is the best link you can provide. You can also link to documentation for the language or library. Highly-upvoted [StackOverflow](https://stackoverflow.com) answers can work as well, but the farther you get from authoritative documentation, the shakier your evidence becomes.
