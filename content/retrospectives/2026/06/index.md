...
This felt like it was a week away for like six weeks, so I'm glad I finally have all the chapters done.
...
Eep, I continue to neglect marketing here, and the numbers are suffering for it. I was desperate to get the last few chapters of the book done, so I focused only on that rather than investing in any marketing.
...
The main vendor I've been working with paid another $7k (bringing me to $17k total) for reports, but they've slowed down on processing reports, so I've mostly stopped searching for new bugs in their code.
...
I like the idea of [_Help this Book_](https://helpthisbook.com/), a web app that allows readers to give feedback directly in your ebook, but I didn't want to store all of my reader feedback with a third party and pay monthly rent.
...
It turns out that it's harder than I expected to navigate AI out of a bad prototype. Once the prototype is bad enough, I have a hard time untangling what the code is even trying to do.

AI seems to have a weird bias to justify whatever code is already present. If I tell the AI that a component seems confusing because it's iterating over the same data three times, it just keeps insisting we have to iterate over the data three times because of X, Y, and Z. But it has a hard time recognizing that X, Y, and Z are artificial constraints, and we could simplify everything if we weren't trying to meet them.
...
That works fine until I get to a complex chunk, like authentication. AI creates a pull request that's 2-5k of confusing code, and that becomes a huge wall. I can't think of a way to break down the feature any further, so I'm stuck with this massive PR, another great blockade.
...
Here's an example. For [Little Moments](https://codeberg.org/mtlynch/little-moments), I'm doing authentication [with magic login emails](https://codeberg.org/mtlynch/little-moments/src/commit/c388029a761628fa48467b25a83d213220394213/docs/design/DESIGN.md#authentication). And for several weeks, I couldn't think of a way to break down that feature without introducing dead code or broken features. I can't implement half a login flow.
...
The thing that makes me wonder if AI is a net positive on this type of work is that I know I would have spotted these opportunities for baby steps earlier had I not been using AI. I would never create a 4k LOC PR and then say, "Hmm, this is pretty big." As the PR grows larger, it becomes more painful to work with, so I naturally see opportunities to break the change into smaller pieces.
...
And AI doesn't force me to create 4k LOC PRs. Now that I recognize how easy it is to generate huge, unmanageable PRs for complex changes, I can change the way I use AI to invest more upfront thinking in how to break features down into tinier changes.
...
- Attract 30k unique visitors to the _Refactoring English_ website.
