...
- I got to [the front page of Hacker News](https://news.ycombinator.com/item?id=47633855) with [an article](/claude-code-found-linux-vulnerability/) that I wrote in a single sitting.
...
My only promotion this month was an article called, ["Which Design Doc Did a Human Write?"](https://refactoringenglish.com/blog/ai-vs-human-design-doc/) It did well [on Lobsters](https://lobste.rs/s/yeoe5q/which_design_doc_did_human_write) but [flopped on Hacker News](https://news.ycombinator.com/item?id=47521257). The /r/programming subreddit rejected it for being "AI-generated," even after I messaged the mods to clarify that the article itself was human-written.

I got the idea for the article from the last live class with _Refactoring English_ readers. We discussed whether AI is good enough to write design docs, and I realized how easy it would be to use AI to generate alternate versions of the design doc I'd just written. It was interesting analyzing the data and seeing the "tells" people spotted in the different design docs.

## Spotting an opportunity to pull a Simon Willison

Simon Willison has been the most popular personal blogger on Hacker News for the last three years straight. I [recently wrote about](https://refactoringenglish.com/blog/2025-hn-top-5/) Willison's underused yet effective strategy for blogging:
...
The only hard part of Willison's strategy is recognizing your moment. If you just summarize tweets and TikToks at random, you probably won't gain much traction. You have to notice when some interesting information is trapped in a Web-hostile format and then port it to the web while it's still fresh.
...
In the talk, Carlini described how he found a remotely-exploitable vulnerability in the Linux kernel that nobody had noticed for 23 years. The surprising part was that anyone could have done what Carlini did. He just asked pointed Claude Code at each file in the Linux kernel source and asked it to report vulnerabilities.
...
It used to be that the average person had _some_ protection from cyberattacks because there's a high cost and barrier to entry to finding security vulnerabilities.
...
And even if you go through all that trouble, you risk "burning" the exploit every time you use it. If someone notices that their system is compromised and traces it back to Syncthing, they might be able to identify your exploit and fix the bug, rendering your exploit worthless.

Compare that to the situation today. You can buy a Claude Code subscription for $100/mo and find critical vulnerabilities just like Carlini did. Claude Code's guardrails make it refuse requests to weaponize vulnerabilities for you, but it won't be long before you can secretly develop exploits with open-weight models without any AI vendor telling you that you can't.
...
What Carlini did is not a fluke. I replicated it to find an undiscovered remote code execution vulnerability in a popular codebase without much effort. I could likely find many more, but the financial value to me is negative because the project has no bug bounty, so I'm spending hours doing free work to coordinate with the vendor on a fix and issue a security advisory.

There isn't some greedy billion dollar company refusing to pay me for vulnerabilities I found. The vulnerability I discovered is in a project run by a lone maintainer of a popular project. It's basically [the "Dependency" xkcd](https://xkcd.com/2347/). They don't have any money to pay for vulnerabilities because they're not getting paid either, though billion-dollar companies are certainly using their code:

{{<img src="dependency_2x.png" max-width="400px" caption="xkcd #2347, [&ldquo;Dependency&rdquo;](https://xkcd.com/2347/)">}}

Eventually, we'll get back to an equilibrium where AI security tools are as cheap and inexpensive as static analysis tools are today. Vendors will use AI to catch security issues before they hit production.

In the short-term, there are a lot of vulnerabilities that suddenly became low-hanging fruit; there's value for attackers to exploit them and not much value for honest researchers to fix them.
...
I found it hard to sit down to write a design doc when [vibe coding is so much more satisfying in the short-term](/retrospectives/2026/03/#ai-assisted-coding-is-becoming-a-problem-for-me). I finally focused and wrote the design doc for my app, Little Moments.

The design doc was, indeed, the hard part. Once that was done, it was pretty straightforward to implement the app. I'm going more slowly than I normally do because I want to maintain high coding standards, so I'm doing lots of iterations with AI tools.

Currently, you can import data you [exported from TinyBeans](https://codeberg.org/mtlynch/tinybeans-export) and render it locally. I don't want to show actual private family photos, so I created dummy data for testing:
...
Little Moments is super fast, and I added [keyboard hotkeys](https://codeberg.org/mtlynch/little-moments/pulls/44) and [mobile swipe gestures](https://codeberg.org/mtlynch/little-moments/pulls/54) to make navigation even easier. I'm excited to finish the app and stop using TinyBeans.
