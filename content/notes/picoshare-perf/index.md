...
Ben has extensive experience with profiling, so he was able to reproduce the issue by creating a new unit test and profiling memory after the test. Ben later showed me a different way of getting the same information that I think is a bit more practical, so I'm going to show that, but you can find Ben's original technique in his pull request.
...
At this point, [Dan Wilhelm](https://danwilhelm.com/) started following the Twitter thread, and even though he's never used Go a day in his life, he rolled up his sleeves and started experimenting with the code on his development machine.
...
This was a huge realization. Because of the difficulty of causing out of memory crashes, I'd used RAM bloat as an approximation for reproducing the problem. But RAM bloat is fine as long as the VM still has enough memory to keep my processes running.
...
Nobody was using the PicoShare server when it crashed, but it did line up with PicoShare's scheduled database maintenance PicoShare. Every seven hours, PicoShare removes expired entries from the database and performs a `VACUUM` to reclaim unused disk space.
...
One of the earliest things I tried was adding a debugging page to PicoShare that showed some of the data from `runtime.ReadMemStats` (I later realized that `net/http/pprof` [did this better](#using-profiling-tools-to-identify-ram-bloat)).
...
In the Twitter thread, a few commenters suggested that Fly might be mounting its temporary directory as a RAM disk. Go's `ParseMultipartForm` function keeps file uploads in the temporary directory, so if Fly mounted it as a RAM disk, that would explain why regular file uploads exhausted RAM.
...
It was a red flag that the Go standard library `ParseMultipartForm` function seeming to consume more memory than its documented limit. I also tried calling `ParseMultipartForm` and then discarding the bytes, which also maxed out RAM. So I tried skipping the Go standard library implementation and [writing an artisanal hand-crafted multipart reader](https://github.com/mtlynch/picoshare/blob/850b9fa50b9c92c060c9267d22194badb15d11b6/handlers/upload.go#L247L322) for my specific scenario.
...
I was curious if uploading data more slowly would have any effect on RAM. I tried uploading from the client end by throttling Chrome to simulate 3G speeds, but PicoShare had the same behavior.

I also tried throttling at the server end to reduce speed of writing to disk, but that didn't do anything either. But I did learn how easy it is to throttle I/O in Go, though. If you're working with an `io.Reader` interface, you can just wrap a `Reader` with a throttled reader like this:
